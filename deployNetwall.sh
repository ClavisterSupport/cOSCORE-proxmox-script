...
convert_ova() {
    local vm_id=$1
    echo "Extracting OVA..."
    mkdir -p "$OVA_EXTRACT_DIR"
    tar -xvf "$TEMP_FILE" -C "$OVA_EXTRACT_DIR"
    VMDK_FILE=$(find "$OVA_EXTRACT_DIR" -name '*.vmdk' | head -n 1)
    if [ -z "$VMDK_FILE" ]; then
...
echo "✅ Clavister VM(s) successfully started."
